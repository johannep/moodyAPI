...
Foam::scalar Foam::RBD::restraints::moodyR::mooringRamp() const
{
	...
	if ( mooringRampTime_>0.0 )
	{
		moorRamp = max(
						0.0, 
						min(
							(model_.time()-startTime_)/(mooringRampTime_), 
							1.0
							) 
						);
	}
...
)
{
	...
	if ( !moodyStarted_ && moored_)
	{
		...
		for ( int ii = 0; ii<noOfAttachments_; ++ii)
		{
			...
			startAttachPoint = bodyPoint0(attachPts_[ii],bodyIDs_[ii]);
		...
		moodyInit(mooringSystemFilename_.c_str(), 3*noOfAttachments_ , &(attachInfo[0]) , model_.prevTime());
	...
	{	
		...
		Info << "solving system between times : " << model_.prevTime() << " and " << model_.time() <<endl;
		moodySolve(&(attachInfo[0]) , &(allForces[0]) , model_.prevTime() , model_.time());
