...
:
    ...
    rigidBodyIterations_(dict.lookupOrDefault<int>("rigidBodyIterations",1)),
...
//    scalarField qDdotPrev = state.qDdot();
    ...
    state.qDdot() = aDamp_*(aRelax_*state.qDdot() + (1 - aRelax_)*qDdotPrev_);
}
...
)
{
    ...
    if (Pstream::master())
    {
	// Include an inner loop of rigidBodyIterations,
	// to help ensure that restraints are converged with new position. 
	int nn=0;
	qDdotPrev_ = motionState_.qDdot();	

	while (nn < rigidBodyIterations_ )  	
	{	
		solver_->solve(tau, fx);
		Info << "Body iteration " << nn << endl; 
        Info << "	with motionState: " << motionState_.q() << endl;
		nn++;
	}
